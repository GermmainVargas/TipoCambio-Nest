...
export class ExchangeRateService {
  ...
  private async getExchangeRate(): Promise<number> {
    try {
      const response = await this.httpService
        .get(`${this.AZA_URL}/task-rate/tipo-de-cambio`)
        .toPromise();
  ...
  private async getExchangeRateEUR(): Promise<number> {
    try {
      const response = await this.httpService
        .get(`${this.AZA_URL}/exchange-rate/tipo-de-cambio`)
        .toPromise();
      return response?.data.tipo_de_cambio;
    } catch (error) {
      throw new HttpException(
        'Error obteniendo tipo de cambio EUR',
        HttpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  private async updateExchangeRates(): Promise<string> {
    ...
    try {
      for (const { Currency, Rate } of exchangeRates) {
        const response = await this.httpService
          .post(
            `${this.API_URL}/SBOBobService_SetCurrencyRate`,
            {
              Currency,
              Rate,
              RateDate: today,
      ...
      throw new HttpException(
        'Error en la actualización del tipo de cambio',
